...
const getDashboard = async (req, res) => {
  try {
    const vp = await query('SELECT * FROM vendor_profiles WHERE user_id = $1', [req.user.id]);
    if (vp.rows.length === 0) return res.status(404).json({ error: 'Vendor profile not found.' });
    const vendorId = vp.rows[0].id;

    const [products, orders, revenue, subscription] = await Promise.all([
      query('SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE is_active = true) as active FROM products WHERE vendor_id = $1', [vendorId]),
      query(`SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE oi.status = 'pending') as pending, COUNT(*) FILTER (WHERE oi.status = 'delivered') as delivered
             FROM order_items oi WHERE oi.vendor_id = $1`, [vendorId]),
      query(`SELECT COALESCE(SUM(oi.total_price), 0) as total_revenue FROM order_items oi
             JOIN orders o ON oi.order_id = o.id WHERE oi.vendor_id = $1 AND o.payment_status = 'paid'`, [vendorId]),
      query(`SELECT * FROM vendor_subscriptions WHERE vendor_id = $1 AND status = 'active' ORDER BY expires_at DESC LIMIT 1`, [vendorId]),
    ]);
...
const getVendorProducts = async (req, res) => {
  try {
    const vp = await query('SELECT id FROM vendor_profiles WHERE user_id = $1', [req.user.id]);
    if (vp.rows.length === 0) return res.status(404).json({ error: 'Vendor profile not found.' });
    ...
    const count = await query('SELECT COUNT(*) FROM products WHERE vendor_id = $1', [vp.rows[0].id]);
...
const createProduct = async (req, res) => {
  try {
    const vp = await query('SELECT id, is_verified FROM vendor_profiles WHERE user_id = $1', [req.user.id]);
    if (vp.rows.length === 0) return res.status(404).json({ error: 'Vendor profile not found.' });
    
    const { name, description, shortDescription, price, comparePrice, categoryId, quantity, brand, images, thumbnail, tags, specifications, sku } = req.body;
    ...
    const result = await query(
      ...
      [vp.rows[0].id, categoryId || null, name, slug, description, shortDescription, price, comparePrice || null, quantity || 0, brand || null, images || '{}', thumbnail || null, tags || '{}', specifications ? JSON.stringify(specifications) : '{}', sku || null]
    );
...
const updateProduct = async (req, res) => {
  try {
    const vp = await query('SELECT id FROM vendor_profiles WHERE user_id = $1', [req.user.id]);
    if (vp.rows.length === 0) return res.status(404).json({ error: 'Vendor not found.' });
    const { id } = req.params;
    ...
    const values = [];
    let i = 1;
    const allowed = ['name', 'description', 'short_description', 'price', 'compare_price', 'category_id', 'quantity', 'brand', 'images', 'thumbnail', 'tags', 'is_active', 'is_featured', 'sku'];
...
const deleteProduct = async (req, res) => {
  try {
    ...
    if (vp.rows.length === 0) return res.status(404).json({ error: 'Vendor not found.' });
    const { id } = req.params;
    await query('UPDATE products SET is_active = false WHERE id = $1 AND vendor_id = $2', [id, vp.rows[0].id]);
